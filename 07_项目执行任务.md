...
- M1 账号与项目资源：登录/刷新/动态路由 + 项目/网关/设备/点位 CRUD。
- M2 采集与查询：MQTT ingest → normalize → pipeline → Timescale/Redis；实时/历史查询可用。
- M3 控制链路：命令下发、回执、审计闭环可用。
- M4 前端联调：pure-admin-thin 登录、路由、权限码与 EMS 业务接口打通。
- M5 验收：满足 PRD DoD 场景与稳定性检查。

## 执行任务清单（按模块）
### 1) 环境与基础设施
- [ ] WSL2 统一运行：PG/Timescale、Redis、MQTT 通过 docker-compose 启动。
- [ ] migrations 初始化表结构；seed 初始化 tenant/project/user/roles/permissions + 示例资产。
- [ ] 本地健康检查脚本与说明（pg_isready/psql/redis-cli）。

### 2) 架构与工程骨架
- [ ] Rust workspace + crates 目录骨架（core/capability/apps）。
- [ ] 模块依赖方向规则落地（domain/api-contract 不反向依赖）。
- [ ] 配置加载与环境变量规范（config crate）。

### 3) 认证与权限
- [ ] JWT access/refresh 登录链路；/login 与 /refresh-token。
- [ ] RBAC 权限码模型与租户隔离 TenantContext。
- [ ] /get-async-routes 动态路由输出（兼容 pure-admin-thin）。

### 4) 项目内资源（元数据）
- [ ] 项目、网关、设备、点位、点位映射 CRUD。
- [ ] 所有表包含 tenant_id 与 project_id，校验归属。
...
### 8) 前端对接
- [ ] 业务接口按 API 契约对齐：/projects/{id}/gateways/devices/points。
- [ ] 权限码与路由 meta 配置稳定。
- [ ] 关键页面联调：登录、项目资源、实时/历史、控制与审计。

### 9) 可观测性与稳定性
- [ ] request_id/trace_id 贯穿日志。
