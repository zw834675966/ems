...
推荐使用 Systemd 管理服务，以确保开机自启和进程守护。

### 3.1 获取部署包
将编译好的二进制文件及相关配置文件部署到服务器 `/opt/ems` 目录。
...
### 3.3 启动服务
```bash
# 1. 配置 Systemd 服务文件 (ems-api.service)
...
- **Web 访问**: 打开浏览器访问 `http://<服务器IP>:8848`（首次初始化账号口令由初始化脚本生成或由部署方指定）
...
```bash
# 备份命令
pg_dump -U ems ems > /backup/ems_$(date +%Y%m%d).sql
```

### 5.2 日志查看
系统采用结构化日志 (JSON/Text)，支持通过 journalctl 查看。
```bash
# 查看实时日志
