...
**协议配置字段（重要）**：
- 网关：`protocolType` + `protocolConfig`（JSON 字符串）
- 设备：`addressConfig`（JSON 字符串；含义取决于网关协议类型）
- 点位映射：`sourceType` + `protocolDetail`（JSON 字符串）

配置规范详见：`05_API契约与前端对接.md` 的 “3.2 协议配置规范（网关/设备/点位映射）”。
