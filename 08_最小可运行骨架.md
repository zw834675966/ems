...
> 注：当前仓库已不止 M0 骨架，实际已落地采集/查询/控制/RBAC/验收脚本等能力；本文件作为 M0 设计基线保留，具体以 `06_开发环境与工作流_WSL2.md`、`07_项目执行任务.md`、`09_完成进度.md`、`10_里程碑完成总结_M0-M5.md` 与代码实现为准。
...
- `EMS_MQTT_USERNAME`：本地 Mosquitto 用户名（默认 `ems`）。
- `EMS_MQTT_PASSWORD`：本地 Mosquitto 密码（默认 `admin123`）。
- `EMS_MQTT_COMMAND_TOPIC_PREFIX`：控制下发主题前缀（可选）。
- `EMS_MQTT_RECEIPT_TOPIC_PREFIX`：回执订阅主题前缀（可选）。
...
说明：动态路由叶子节点 `children` 为空时省略该字段，避免前端菜单过滤。
