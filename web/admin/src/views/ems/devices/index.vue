...
const rules = reactive<FormRules>({
  ...
  name: [
    ...
    { min: 2, max: 50, message: "设备名称长度应为 2-50 个字符", trigger: "blur" }
  ]
});
...
const submit = async () => {
  ...
  try {
    ...
    const addressConfig = buildAddressConfig(selectedGateway.value?.protocolType, form);
      ...
      <el-skeleton :rows="5" animated :loading="loading && filteredData.length === 0">
        <template #default>
          ...
          >
            ...
            <el-table-column prop="addressConfig" label="从站地址" width="120" align="center">
              <template #default="{ row }">
            ...
            <template #empty>
              <el-empty description="请先选择项目，或暂无设备数据" :image-size="120">
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top" class="mt-4">
        ...
        <el-form-item label="设备名称" prop="name">
          <el-input v-model="form.name" placeholder="例如：1号变压器" maxlength="50" show-word-limit />
        </el-form-item>
        ...
        <template v-if="isModbusGateway">
          <div class="bg-secondary p-4 rounded-lg mb-6 text-sm">
            ...
            <el-form-item label="从站地址 (Unit ID)" required class="mb-0">
              <el-input-number v-model="form.unitId" :min="1" :max="247" class="w-full" />
